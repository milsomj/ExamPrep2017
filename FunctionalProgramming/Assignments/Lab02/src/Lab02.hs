...
getMeasured rqdDistance [] = []
getMeasured rqdDist (x:xs)
            |distance x >= rqdDist = [x]
            |otherwise = x : (getMeasured (rqdDist - distance x) xs)
...
findFastest :: Distance -- desired distance of sub-SegPath
            ...
            -> Maybe -- fail if path too short
                 ...
                 , Speed ) -- fastest Speed
findFastest rqdDist [x] = Just ((getMeasured rqdDist [x] ), pathSpeed (getMeasured rqdDist [x]))

findFastest rqdDist p
            | pathDistance p < rqdDist = Nothing
            | otherwise = Just((helper p rqdDist p) , pathSpeed((helper p rqdDist p)))

helper :: SegPath->Distance->SegPath->SegPath
helper currentBest rqdDist [x]
            | pathSpeed [x] < pathSpeed (getMeasured rqdDist currentBest) = [x]
            | otherwise = currentBest
helper currentBest rqdDist (x:xs)
            | pathSpeed (getMeasured rqdDist (x:xs)) < pathSpeed (getMeasured rqdDist currentBest) = getMeasured rqdDist (helper (x:xs) rqdDist xs)
            | otherwise = getMeasured rqdDist (helper currentBest  rqdDist xs)
